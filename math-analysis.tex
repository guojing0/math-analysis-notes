...
\begin{document}
    ...
    \subsection{Metric Space}
        ...
        \subsection{Open Ball and Neighborhood}
        ...
        $U$ is a \textit{neighborhood} of $x$ if $x \in U^{o}$. Open neighborhood of $x$ is any open set containing $x$.
        ...
        \textbf{Open cover}: An open cover of a set $Z$ is a collection $(U_{i}, i \in I)$ of open sets, such that $Z \subset \bigcup_{i \in I} U_{i}$.
        
        Not-so-useful definition: A set $C \subset S$ is compact if for any open cover, there exists a finite subcover that contains $C$.
        
        Examples:
        
        \begin{enumerate}
            \item $\mathbb{R}^{2}$ is not a compact set.
            \item Any finite set is compact, or if $C_{1}, C_{2}, \cdots, C_{n}$ is compact, then $\bigcup_{i = 1}^{n} C_{i}$ is also compact.
            \item $S$ is a discrete space, $C \subset S$ is compact. $\forall c \in C$, $\{ c \}$ is open, so $\bigcup_{c \in C} \{ c \} = C \Rightarrow$ $C$ is finite so it is compact. Therefore in discrete space, all finite sets are compact.
        \end{enumerate}
        
        \begin{theorem}
            $S$ is a topological space and $C$ is a compact set in $S$ and $z \in C$ is a closed set, then $z$ is compact.
        \end{theorem}
        ...
        A topological space is \textit{Hausdorff} if any two distinct points $x, y \in S$ are separated by open sets, that is, $x \in U, y \in V$, $U, V \subset S$, and $U \cap V = \emptyset$.
        ...
        \begin{proof}
            We have two distinct points $x$ and $y$ in topological space, and $\epsilon = d(x, y) > 0$.
            
            Suppose point $z \in B_{\frac{\epsilon}{3}} (x) \cap B_{\frac{\epsilon}{3}} (y)$, so $d(x, z) < \frac{\epsilon}{3}$ and $d(y, z) < \frac{\epsilon}{3}$.
            
            Due to triangular inequity, $d(x, y) < \frac{\epsilon}{3} + \frac{\epsilon}{3} < \epsilon$, which implies such point $z$ does not exist, that is, intersection does not exist. 
        \end{proof}
